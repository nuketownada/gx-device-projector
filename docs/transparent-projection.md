...
| A will fired while logic was **down** (non-retained → lost) → a dead board stays `/Connected=1` forever, telemetry frozen | The board's will is a **retained `device/<id>/online=0`** (it publishes retained `1` on connect, before the announce). Because it's retained, every logic incarnation gets the current truth the instant it subscribes; `"0"` arms the same disconnect grace (single-writer: `online` never sets `Connected=1` — that comes only from a processed registration). |

**Liveness — closed.** Non-retained registration replaced retained-`Status`'s *durability* role; retained `device/<id>/online` replaces its *liveness* role, the last piece. A board that dies while logic is down (or during the grace with a logic restart) is no longer stranded: on restart, logic subscribes `<ns>/+/online`, the broker delivers the retained `0`, and the disconnect commits grace-after-reconcile. Proven by `tests/bench_liveness.sh` — kills the board with logic down, so the retained `online=0` is the *only* surviving signal, and asserts `/Connected` commits to `0` (with values invalidated) rather than sticking at `1`. With this, every role retained-`Status` ever played — durability (state daemon), delivery (reg-timeout retry), liveness (retained `online`), flap suppression (debounce) — has a named owner and a named test.
