...
>
> **Why:** a GX is a reactive control system that acts on dbus *edges*. Upstream seeds default/persisted values on (re)start, so a driver restart or a stale value can fire a phantom edge that systemcalc / dbus-generator act on — real collateral, not cosmetic. Here, projected state has the lifetime of the board's connection and only the board authors it.
>
> **Shape** (see [`statehost/`](statehost/)): a **durable state daemon** owns the dbus services — they survive a logic redeploy with zero blip — and a **restartable logic daemon** bridges MQTT ⇄ the state daemon. Registration is **non-retained with board-authored `init`**, plus a **state cookie** so boards re-announce if the durable state is ever lost. v1 (retained) clients keep working through migration ([§8](docs/transparent-projection.md#8-migration--sequencing)).

This VenusOS driver works with the Victron [dbus-flashmq](https://github.com/victronenergy/dbus-mqtt) gateway to let Wi-Fi edge devices (ESP32, Arduino, Raspberry Pi) self-register on the dbus over MQTT — no dedicated per-device driver required.
